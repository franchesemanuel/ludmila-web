...
class Turno(models.Model):
    ...
    def clean(self):
        ...
        # ── Solapamiento: duración completa para turnos nuevos, hora exacta para existentes ──
        config = ConfiguracionAgenda.objects.first()
        intervalo = config.intervalo_minutos if config else 30

        if not self.pk:
            # Turno nuevo: verificar solapamiento por duración real
            ...
            for t in Turno.objects.filter(fecha=self.fecha, cancelado=False).select_related("servicio"):
                try:
            ...
            # Turno existente (admin confirma, edita nota, etc.): solo bloquear hora exacta duplicada
